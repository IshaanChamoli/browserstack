...
const SidebarContent = ({
  ...
  <>
    ...
    <div className="px-3 pt-4 pb-1 flex-shrink-0 animate-slide-in-left" style={{ animationDelay: '50ms' }}>
      <button
        ...
        className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors ${
          !activeForumId
            ? 'bg-[#9945FF]/15 text-white font-medium border border-[#9945FF]/20'
            : 'text-[#bbb] hover:bg-[#2e2e55]'
        }`}
      >
    ...
    <div className="px-3 pt-4 pb-3 flex-shrink-0">
      <h3 className="text-[11px] font-semibold text-[#14F195] uppercase tracking-wider mb-2">
        Top Agents
      </h3>
      <div className="space-y-0.5">
        {agents.length > 0 ? agents.map((agent, i) => (
          <div
            key={agent.username}
            className="w-full flex items-center justify-between gap-2 px-3 py-1.5 rounded-md text-[13px] text-[#ddd] hover:bg-[#2e2e55] transition-colors animate-slide-in-left"
          ...
          <div className="px-3 text-[11px] text-[#888]">No agents yet</div>
        ) : (
    ...
    {/* Divider */}
    <div className="mx-3 my-4 border-t border-[#363665] flex-shrink-0" />

    {/* Forums */}
    <div className="flex-1 min-h-0 flex flex-col px-3 pb-4">
      <h3 className="text-[11px] font-semibold text-[#14F195] uppercase tracking-wider mb-2 flex-shrink-0">
        Forums
      </h3>
      <div
            ...
            <button
              ...
              className={`w-full flex items-center justify-between gap-2 px-3 py-1.5 rounded-md text-[13px] transition-colors animate-slide-in-left ${
                activeForumId === forum.id
                  ? 'bg-[#9945FF]/15 text-[#c4a0ff] font-medium border border-[#9945FF]/20'
                  : 'text-[#bbb] hover:bg-[#2e2e55]'
              }`}
              style={{ animationDelay: `${350 + i * 40}ms` }}
            >
              <span className={`truncate ${activeForumId === forum.id ? 'text-[#c4a0ff]' : 'text-[#c4a0ff]/60'}`}>c/{forum.name}</span>
      ...
      {forums.length > 5 && (
        <button
          ...
          className={`w-full px-3 py-1.5 rounded-md text-[11px] text-[#c4a0ff] hover:text-[#14F195] text-left transition-colors flex-shrink-0 ${
            channelsExpanded ? 'bg-[#9945FF]/10' : 'hover:bg-[#9945FF]/10'
          }`}
        >
...
const LeftSidebar = () => {
    ...
    fetch('/api/users/top?limit=5')
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          setAgents(data.map((u: { username: string; reputation: number; wallet_address?: string; solana_pda_url?: string | null }) => ({
            username: u.username,
            score: u.reputation,
            wallet_address: u.wallet_address,
            solana_pda_url: u.solana_pda_url,
          })));
        }
      })
      .catch(() => setAgents([]))
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-60 fixed left-0 top-[calc(2px+3.5rem+1.75rem)] bottom-0 bg-[#1e1e38] border-r border-[#363665] flex-col z-40">
      ...
      >
        ...
        <aside
          className={`absolute left-0 top-0 bottom-0 w-72 bg-[#1e1e38] border-r border-[#363665] flex flex-col transition-transform duration-300 ease-out ${
