...
const QuestionList = () => {
  ...
  return (
    <div className="py-4 px-4 md:py-6 md:px-6">
      ...
      <p key={`count-${searchQuery}-${forumId}-${activeTab}`} className="text-sm text-[#aaa] mb-4 min-h-[20px] animate-fade-in transition-opacity duration-300" style={{ opacity: totalQuestions !== null ? 1 : 0 }}>
        {totalQuestions !== null ? `${totalQuestions.toLocaleString()} questions` : '\u00A0'}
      </p>

      {/* Filter tabs */}
      <div className="flex items-center gap-1 border-b border-[#363665] pb-3">
        {tabs.map((tab) => (
          <button
            key={tab.id}
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-[#bbb] text-sm animate-fade-in">No questions found.</div>
      ) : (
        questions.map((q, i) => (
          <div
            key={q.id}
      ...
      {totalPages > 1 && !loading && (
        <div className="flex items-center justify-center gap-1.5 py-8 animate-fade-in">
          <button
            ...
            className="px-3 py-1.5 text-sm border border-[#363665] rounded text-[#bbb] hover:bg-[#2e2e55] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Prev
          </button>
          ...
          {paginationPages()[0] > 1 && (
            <>
              <button
                ...
                className="min-w-[36px] px-2 py-1.5 text-sm border border-[#363665] rounded text-[#bbb] hover:bg-[#2e2e55] transition-colors"
              >
                1
              </button>
              {paginationPages()[0] > 2 && (
                <span className="px-1 text-sm text-[#aaa]">...</span>
              )}
            </>
          )}
          ...
          {paginationPages().map((page) => (
            <button
              ...
              className={`min-w-[36px] px-2 py-1.5 text-sm border rounded transition-colors ${
                currentPage === page
                  ? 'bg-[#9945FF] border-[#9945FF] text-white font-medium'
                  : 'border-[#363665] text-[#bbb] hover:bg-[#2e2e55]'
              }`}
            >
              {page}
            </button>
